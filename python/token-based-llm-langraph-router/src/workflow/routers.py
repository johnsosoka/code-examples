...
def query_router(state: State):
    ...
    prompt = PromptTemplate(
        template="Determine if the user query requires a simple or advanced model. An 'advanced' request might require multiple \
    steps like retrieving an order ID and looking up shipping information, whereas a 'simple' request can handle more \
    straightforward queries. return either 'simple' or 'advanced'. Do not explain your reasoning Your only task is to \
    determine where to route the user query. \
    user_query: {input}",
